<?php
/**
 * EvolveWP ClientJourney Development - Credits & Contributors
 *
...
/**
 * EvolveWP_CJ_Admin_Development_Credits Class.
 *
 * @since   1.0.0
 * @version 1.2.0
 */
class EvolveWP_CJ_Admin_Development_Credits {
    ...
    public static function output() {
        ...
        wp_enqueue_style('evolvewp-clientjourney-accordion-table', EVOLVEWP_CJ_PLUGIN_URL . 'assets/css/accordion-table.css', array(), EVOLVEWP_CJ_VERSION);
        wp_enqueue_script('evolvewp-clientjourney-accordion-table', EVOLVEWP_CJ_PLUGIN_URL . 'assets/js/accordion-table.js', array('jquery'), EVOLVEWP_CJ_VERSION, true);
        ?>
        
        <div class="evolvewp-clientjourney-credits-container">
            <div class="evolvewp-clientjourney-layout">
                ...
                <div class="evolvewp-clientjourney-table-container">
                    ...
                    <div class="tablenav top">
                        <div class="alignleft actions">
                            <select id="category-filter">
                                ...
                                <option value="library"><?php esc_html_e('Libraries', 'evolvewp-clientjourney'); ?></option>
                                <option value="inspiration"><?php esc_html_e('Inspiration', 'evolvewp-clientjourney'); ?></option>
                                <option value="service"><?php esc_html_e('Services', 'evolvewp-clientjourney'); ?></option>
                            </select>
                            <input type="search" id="contributor-search" placeholder="<?php esc_attr_e('Search...', 'evolvewp-clientjourney'); ?>">
                        </div>
                    </div>
                    ...
                    <div class="wp-list-table widefat fixed striped">
                        <div class="table-header" style="display: flex; background: #f1f1f1; padding: 12px 15px; font-weight: 600; border-bottom: 1px solid #c3c4c7;">
                            <div style="flex: 2;"><?php esc_html_e('Name', 'evolvewp-clientjourney'); ?></div>
                            <div style="flex: 1;"><?php esc_html_e('Type', 'evolvewp-clientjourney'); ?></div>
                            <div style="flex: 1;"><?php esc_html_e('License', 'evolvewp-clientjourney'); ?></div>
                            <div style="flex: 1;"><?php esc_html_e('Status', 'evolvewp-clientjourney'); ?></div>
                        </div>
                    </div>

                    <div class="evolvewp-clientjourney-accordion-table">
                            ...
                            <div class="accordion-row" data-category="<?php echo esc_attr($contributor['category']); ?>" data-contributor-id="<?php echo esc_attr($id); ?>">
                                ...
                                <div class="accordion-content">
                                    <div class="contributor-meta">
                                        <div>
                                            <strong><?php esc_html_e('Description:', 'evolvewp-clientjourney'); ?></strong><br>
                                            <?php echo esc_html($contributor['description']); ?>
                                        </div>
                                        <div>
                                            <strong><?php esc_html_e('Used In:', 'evolvewp-clientjourney'); ?></strong><br>
                                    ...
                                    <div class="contributor-actions">
                                        ...
                                        <?php if ($contributor['website']): ?>
                                            <a href="<?php echo esc_url($contributor['website']); ?>" class="button" target="_blank">
                                                <?php esc_html_e('Visit Website', 'evolvewp-clientjourney'); ?>
                                            </a>
                                        <?php endif; ?>
                                        <?php if ($contributor['github']): ?>
                                            <a href="<?php echo esc_url($contributor['github']); ?>" class="button" target="_blank">
                                                <?php esc_html_e('GitHub', 'evolvewp-clientjourney'); ?>
                                            </a>
                                        <?php endif; ?>
                                    </div>
                                </div>
                ...
                <div class="evolvewp-clientjourney-sidebar">
                    <div class="evolvewp-clientjourney-details-container">
                        ...
                        <?php else: ?>
                            <div class="details-placeholder">
                                <p><?php esc_html_e('Select a contributor to view details', 'evolvewp-clientjourney'); ?></p>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
        ...
        jQuery(document).ready(function($) {
            ...
            if (useAjax) {
                $('.accordion-header').on('click', function(e) {
                    ...
                    $('.evolvewp-clientjourney-details-container').html('<div class="details-loading"><span class="spinner is-active" style="float:none;"></span></div>');
                    
                    $.post(ajaxurl, {
                        action: 'evolvewp_cj_get_contributor_details',
                        contributor_id: contributorId,
                        nonce: '<?php echo esc_attr(wp_create_nonce('evolvewp_cj_contributor_details')); ?>'
                    }, function(response) {
                        console.log('Response:', response);
                        if (response.success) {
                            $('.evolvewp-clientjourney-details-container').html(response.data.html);
                        } else {
                            $('.evolvewp-clientjourney-details-container').html('<p>Error loading details</p>');
                        }
                    }).fail(function(xhr, status, error) {
                        console.error('AJAX Error:', status, error);
                        $('.evolvewp-clientjourney-details-container').html('<p>Error: ' + error + '</p>');
                    });
                });
            }
        });
    ...
    private static function render_contributor_details($contributor) {
        ...
        <div class="section-content">
            ...
            <div class="detail-group">
                <label><?php esc_html_e('Description:', 'evolvewp-clientjourney'); ?></label>
                <p><?php echo esc_html($contributor['description']); ?></p>
            </div>
            
            <div class="detail-group">
                <label><?php esc_html_e('Full Details:', 'evolvewp-clientjourney'); ?></label>
                <p><?php echo esc_html($contributor['details']); ?></p>
            </div>
            
            <div class="detail-group">
                <label><?php esc_html_e('Category:', 'evolvewp-clientjourney'); ?></label>
                <span class="type-badge type-<?php echo esc_attr($contributor['category']); ?>">
                    <?php echo esc_html(ucfirst($contributor['category'])); ?>
                </span>
            </div>
            ...
            <div class="detail-group">
                <label><?php esc_html_e('License:', 'evolvewp-clientjourney'); ?></label>
                <span class="license-badge"><?php echo esc_html($contributor['license']); ?></span>
            </div>
            
            <?php if ($contributor['stats']): ?>
                <div class="detail-group">
                    <label><?php esc_html_e('Statistics:', 'evolvewp-clientjourney'); ?></label>
            ...
            <div class="detail-actions">
                ...
                <?php if ($contributor['github']): ?>
                    <a href="<?php echo esc_url($contributor['github']); ?>" class="button" target="_blank">
                        <?php esc_html_e('View on GitHub', 'evolvewp-clientjourney'); ?>
                    </a>
                <?php endif; ?>
            </div>
        </div>
    ...
    public static function ajax_get_contributor_details() {
        ...
        if (!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'evolvewp_cj_contributor_details')) {
    ...
    private static function get_contributors() {
        return array(
            ...
            'woocommerce' => array(
                ...
                'details' => 'Created by Automattic. EvolveWP ClientJourney draws inspiration from WooCommerce\'s plugin architecture, including REST API patterns, background processing, and asset management.',
            ...
            'github' => array(
                ...
                'details' => 'Owned by Microsoft Corporation. GitHub provides version control, issue tracking, and CI/CD integration for EvolveWP ClientJourney development.',
                'used_in' => 'Version control, documentation sync, CI/CD',
                'website' => 'https://github.com/',
                'github' => 'https://github.com/ryanbayne/evolvewp-clientjourney',
                'logo' => '',
...
EvolveWP_CJ_Admin_Development_Credits::init();

add_action('wp_ajax_evolvewp_cj_get_contributor_details', array('EvolveWP_CJ_Admin_Development_Credits', 'ajax_get_contributor_details'));
