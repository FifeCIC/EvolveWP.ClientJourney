/**
 * evolvewp-clientjourney Development Tabs JavaScript
 *
...
(function($) {
    ...
    function initdevelopment() {
        console.log('evolvewp-clientjourney Development Tabs JS loaded');
    ...
    function initAccordions() {
        ...
        if ($('.evolvewp-clientjourney-accordion-container').length > 0) {
            ...
            $('.collapse-all').off('click').on('click', function() {
                $('.evolvewp-clientjourney-accordion-content').slideUp();
                $('.evolvewp-clientjourney-accordion-icon')
    ...
    function initUILibrary() {
        ...
        if (typeof evolvewp-clientjourneyUILibrary !== 'undefined') {
            // UI Library JavaScript is loaded
            console.log('evolvewp-clientjourneyUILibrary object detected');
        }
    }
...
jQuery(document).ready(function($) {
    ...
    $('.refresh-github-data').on('click', function(e) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'evolvewp_cj_refresh_github_data',
                nonce: evolvewp_cj_dev_tabs.nonce
            },
    ...
    $('.section-toggle-checkbox').on('change', function() {
        ...
        localStorage.setItem('evolvewp_cj_ui_library_sections', JSON.stringify(sectionStates));
    });
    ...
    var savedStates = localStorage.getItem('evolvewp_cj_ui_library_sections');
    if (savedStates) {
        try {
    ...
    $('.move-to-assets').on('click', function() {
        ...
        if (confirm(evolvewp-clientjourneyData.confirmMoveFile || 'Are you sure you want to move this file to the assets folder?')) {
            $btn.prop('disabled', true).text(evolvewp-clientjourneyData.movingText || 'Moving...');
            
            setTimeout(function() {
                alert(evolvewp-clientjourneyData.moveFileMessage || 'File move functionality would be implemented here');
                $btn.prop('disabled', false).text(evolvewp-clientjourneyData.moveToAssetsText || 'Move to Assets');
            }, 1000);
        }
    });
    ...
    $('.delete-file').on('click', function() {
        ...
        if (confirm(evolvewp-clientjourneyData.confirmDeleteFile || 'Are you sure you want to delete this file? This action cannot be undone.')) {
            $btn.prop('disabled', true).text(evolvewp-clientjourneyData.deletingText || 'Deleting...');
            
            setTimeout(function() {
                alert(evolvewp-clientjourneyData.deleteFileMessage || 'File delete functionality would be implemented here');
                $btn.prop('disabled', false).text(evolvewp-clientjourneyData.deleteText || 'Delete');
            }, 1000);
        }
    });
    ...
    if ($('.feature-status-tab').length > 0) {
        ...
        // Initialize evolvewp-clientjourney-accordion functionality for feature status
        $('.evolvewp-clientjourney-accordion-header').on('click', function() {
            var $content = $(this).next('.evolvewp-clientjourney-accordion-content');
            $content.slideToggle();
            $(this).find('.evolvewp-clientjourney-accordion-icon')
    ...
    $('#load-log-file').on('click', function() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'evolvewp_cj_load_log_file',
                log_file: logFile,
                security: evolvewp_cj_dev_tabs.nonce
            },
            success: function(response) {
                if (response.success) {
    ...
    // Tasks functionality
    if (typeof evolvewp-clientjourneyTasksData !== 'undefined') {
        $('.start-task').on('click', function(e) {
            e.preventDefault();
            var taskId = $(this).data('task-id');
            var taskSource = $(this).data('task-source');
            
            localStorage.setItem('evolvewp_cj_current_task_id', taskId);
            localStorage.setItem('evolvewp_cj_current_task_source', taskSource);
            
            $(this).closest('tr').attr('data-status', 'in-progress');
            
            if (taskSource !== 'github') {
                var taskData = null;
                for (var i = 0; i < evolvewp-clientjourneyTasksData.tasks.length; i++) {
                    if (evolvewp-clientjourneyTasksData.tasks[i].id === taskId) {
                        taskData = evolvewp-clientjourneyTasksData.tasks[i];
                        break;
                    }
                }
                ...
                if (taskData) {
                    ...
                    $.ajax({
                        url: evolvewp_cj_dev_tabs.ajaxurl,
                        type: 'POST',
                        data: {
                            action: 'evolvewp_cj_create_github_issue',
                            nonce: evolvewp_cj_dev_tabs.nonce,
                        ...
                        success: function(response) {
                            if (response.success) {
                                localStorage.setItem('evolvewp_cj_GITHUB_issue_number', response.data.number);
                                localStorage.setItem('evolvewp_cj_GITHUB_issue_url', response.data.html_url);
                                window.location.href = 'admin.php?page=evolvewp_cj_development&tab=current_task';
                            } else {
