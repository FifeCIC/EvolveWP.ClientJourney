<?php
/**
 * EvolveWP ClientJourney Asset Queue System
 * 
 * Centralized asset enqueueing based on page detection
 * 
 * @package EvolveWP ClientJourney/Assets
 * @version 1.2.0
 */
...
class EvolveWP_CJ_Asset_Queue {
    ...
    public function __construct() {
        if (!class_exists('EvolveWP_CJ_Asset_Manager')) {
            require_once EVOLVEWP_CJ_PLUGIN_DIR_PATH . 'assets/manage-assets.php';
        }
        
        global $evolvewp_cj_assets;
        $this->asset_manager = $evolvewp_cj_assets;
    ...
    private function enqueue_style($name, $asset) {
        ...
        $deps = array_map( function( $dep ) {
            ...
            if ( 0 === strpos( $dep, 'evolvewp-clientjourney-' ) || 0 === strpos( $dep, 'wp-' ) ) {
                return $dep;
            }
            return 'evolvewp-clientjourney-' . $dep;
        }, $asset['dependencies'] );
        
        wp_enqueue_style(
            'evolvewp-clientjourney-' . $name,
            $url,
            $deps,
            EVOLVEWP_CJ_VERSION,
            'all'
    ...
    private function enqueue_script($name, $asset) {
        ...
        // Prefix dependency handles to match registered handle format.
        $deps = array_map( function( $dep ) {
            if ( 0 === strpos( $dep, 'evolvewp-clientjourney-' ) || 0 === strpos( $dep, 'wp-' ) || 'jquery' === $dep ) {
                return $dep;
            }
            return 'evolvewp-clientjourney-' . $dep;
        }, $asset['dependencies'] );
    ...
    public function missing_assets_notice() {
        ...
        <div class="notice notice-warning is-dismissible">
            <p>
                <strong>EvolveWP ClientJourney:</strong>
                <?php
                /* translators: %d: Number of missing asset files */
                echo esc_html(sprintf(_n('%d asset file is missing', '%d asset files are missing', $count, 'evolvewp-clientjourney'), $count));
                ?>
            </p>
...
if (!class_exists('EvolveWP_CJ_Asset_Queue_Instance')) {
    class EvolveWP_CJ_Asset_Queue_Instance {
        private static $instance = null;
        
        public static function get_instance() {
            if (self::$instance === null) {
                self::$instance = new EvolveWP_CJ_Asset_Queue();
            }
            return self::$instance;
        }
    }
    
    EvolveWP_CJ_Asset_Queue_Instance::get_instance();
}
