...
```php
$scheduler = EvolveWP_CJ_Task_Scheduler::instance();
...
- `$group` (string) - Group name for organization (default: 'evolvewp-clientjourney')
...
- `$group` (string) - Group name (default: 'evolvewp-clientjourney')
...
add_action('daily_cleanup', function() {
    // Delete old logs
    global $wpdb;
    $wpdb->query("DELETE FROM {$wpdb->prefix}evolvewp_cj_logs WHERE created_at < DATE_SUB(NOW(), INTERVAL 30 DAY)");
});
```
...
## Migration from EvolveWP_CJ_Background_Process

### Old Way

```php
class My_Process extends EvolveWP_CJ_Background_Process {
...
// ✅ Good - specific name
$scheduler->schedule_single('evolvewp_cj_process_user_data', $args);
```
