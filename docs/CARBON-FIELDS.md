...
add_action('evolvewp_cj_carbon_fields_register', 'my_theme_options');
function my_theme_options() {
    Container::make('theme_options', __('My Settings'))
        ->set_page_parent('evolvewp-clientjourney-settings')
        ->add_fields(array(
...
```php
add_action('evolvewp_cj_carbon_fields_register', 'my_post_meta');
...
```php
// Create options page
EvolveWP_CJ_Carbon_Fields::create_options_page('My Settings', 'my-settings', 'evolvewp-clientjourney-settings');

// Create post meta
EvolveWP_CJ_Carbon_Fields::create_post_meta('Product Info', array('product'));

// Create term meta
EvolveWP_CJ_Carbon_Fields::create_term_meta('Category Fields', array('category'));

// Create user meta
EvolveWP_CJ_Carbon_Fields::create_user_meta('User Profile');
```
...
1. **Use Prefixes**: Always prefix field names (`evolvewp_cj_field_name`)
...
- GitHub: https://github.com/ryanbayne/evolvewp-clientjourney/issues
