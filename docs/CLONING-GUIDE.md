...
> How to create a new EvolveWP ecosystem plugin from the EvolveWPClientJourney.
...
You need:
- The `EvolveWP.EvolveWPClientJourney` directory (the cloning source)
...
**The EvolveWPClientJourney must never be activated** alongside Core or any real
...
|---|---|---|---|
| 1 | `EvolveWP\ClientJourney\` | `{Namespace}\` | `EvolveWP\OpsStudio\` |
| 2 | `EVOLVEWP_CJ_` | `{CONSTANT_PREFIX_}` | `EVOLVEWP_OPS_` |
| 3 | `EvolveWP_CJ_` | `{Legacy_Class_Prefix_}` | `EvolveWP_Ops_` |
| 4 | `evolvewp_cj_` | `{function_prefix_}` | `evolvewp_ops_` |
| 5 | `evolvewp-clientjourney` | `{slug}` | `evolvewp-opsstudio` |
| 6 | `EvolveWPClientJourney` | `{ClassName}` | `EvolveWPOpsStudio` |
| 7 | `EvolveWP ClientJourney` | `{Display Name}` | `EvolveWP OpsStudio` |

### Why This Order?

- **Namespace (#1) before class name (#6)**: The namespace token
  `EvolveWP\ClientJourney\` contains `EvolveWPClientJourney`. If the class
  name replacement runs first, it becomes `EvolveWP\EvolveWPOpsStudio\` — wrong.
- **Legacy class prefix (#3) early**: `EvolveWP_CJ_` is the prefix for
...
Use the PowerShell script in the EvolveWPClientJourney directory:

```powershell
cd c:\wamp64\www\Ecosystem\wp-content\plugins\EvolveWP.EvolveWPClientJourney
...
Copy all files from `EvolveWP.EvolveWPClientJourney/` into the target directory.
**Preserve the target's `.git` folder** — do not overwrite or delete it.

```
robocopy "EvolveWP.EvolveWPClientJourney" "EvolveWPClientJourney" /E /XD .git /IS /IT
```

### Step 2 — Rename the main plugin file

```
evolvewp-clientjourney.php  →  evolvewp-clientjourney.php
```
...
```bash
git add .
git commit -m "Initial clone from EvolveWPClientJourney"
...
- Was the legacy class prefix replacement (#3) run? Search for `EvolveWP_CJ_`
...
`EvolveWP ClientJourney` (case-sensitive) — any remaining instances are bugs.
