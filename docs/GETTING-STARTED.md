...
1. Download the latest release from [GitHub](https://github.com/ryanbayne/evolvewp-clientjourney/releases)
2. Extract to `wp-content/plugins/`
3. Rename folder from `evolvewp-clientjourney-main` to `wpseed`

### Method 3: WP-CLI
```bash
wp plugin install https://github.com/ryanbayne/evolvewp-clientjourney/archive/main.zip --activate
```
...
- **Command Line**: 
  ```bash
  # Linux/Mac
  find . -type f -name "*.php" -exec sed -i 's/evolvewp-clientjourney/myawesomeplugin/g' {} +
  
  # Windows (PowerShell)
  Get-ChildItem -Recurse -Filter *.php | ForEach-Object { (Get-Content $_.FullName) -replace 'evolvewp-clientjourney', 'myawesomeplugin' | Set-Content $_.FullName }
  ```
...
```php
<?php
class MyAwesomePlugin_REST_Books extends EvolveWP_CJ_REST_Controller {
...
class MyAwesomePlugin_Settings_Books extends EvolveWP_CJ_Settings_Page {
...
2. Check `wp_options` for `evolvewp_cj_db_version`
3. Manually run: `EvolveWP_CJ_Install::install()`
...
- [GitHub Discussions](https://github.com/ryanbayne/evolvewp-clientjourney/discussions)
- [Documentation](https://github.com/ryanbayne/evolvewp-clientjourney/wiki)
...
### Key Classes
- `EvolveWP_CJ_Install` - Installation and setup
- `EvolveWP_CJ_REST_Controller` - REST API base
- `EvolveWP_CJ_Logger` - Logging system
- `EvolveWP_CJ_AI_Assistant` - AI integration
- `EvolveWP_CJ_Settings_Page` - Settings framework

### Key Functions
- `evolvewp_cj_log()` - Log messages
- `evolvewp_cj_get_option()` - Get plugin option
- `evolvewp_cj_help_tip()` - Add tooltip
- `evolvewp_cj_clean()` - Sanitize data

### Hooks
- `evolvewp_cj_loaded` - Plugin loaded
- `evolvewp_cj_init` - Plugin initialized
- `evolvewp_cj_settings_save_{tab}` - Settings saved
- `evolvewp_cj_admin_mainviews` - Add admin views
