...
$license_manager = new EvolveWP_CJ_License_Manager(
...
$extension_installer = new EvolveWP_CJ_Extension_Installer(
...
```php
define( 'EVOLVEWP_CJ_LICENSE_SERVER', 'https://your-server.com' );
define( 'EVOLVEWP_CJ_PRODUCT_ID', 'evolvewp-clientjourney-plugin' );
```
...
### Activate License
```
POST /wp-json/evolvewp-clientjourney-license/v1/activate
```
...
### Deactivate License
```
POST /wp-json/evolvewp-clientjourney-license/v1/deactivate
```

### Check License Status
```
POST /wp-json/evolvewp-clientjourney-license/v1/check
```
...
### Check for Updates
```
POST /wp-json/evolvewp-clientjourney-license/v1/update_check
```
...
### Get Plugin Info
```
POST /wp-json/evolvewp-clientjourney-license/v1/plugin_info
```

### Get License Info
```
POST /wp-json/evolvewp-clientjourney-license/v1/info
```
...
### List Extensions
```
GET /wp-json/evolvewp-clientjourney-extensions/v1/list
```
...
### Download Extension
```
POST /wp-json/evolvewp-clientjourney-extensions/v1/download
```
...
```php
$license_manager = new EvolveWP_CJ_License_Manager(
    EVOLVEWP_CJ_LICENSE_SERVER,
    EVOLVEWP_CJ_PRODUCT_ID,
    EVOLVEWP_CJ_VERSION
);
...
```php
$installer = new EvolveWP_CJ_Extension_Installer( EVOLVEWP_CJ_LICENSE_SERVER );
...
```php
check_admin_referer( 'evolvewp_cj_license_action' );
```
...
```php
add_filter( 'evolvewp_cj_license_validation', function( $is_valid, $license_key ) {
...
```php
add_filter( 'evolvewp_cj_update_check_interval', function( $interval ) {
...
```php
add_filter( 'evolvewp_cj_extensions_list', function( $extensions ) {
...
2. Check update check transient: `evolvewp_cj_license_check_{product_id}`
