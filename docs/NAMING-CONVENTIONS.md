...
|---|---|---|
| PHP constants | `EVOLVEWP_CJ_` | `EVOLVEWP_VERIFIER_` |
| PHP functions | `evolvewp_cj_` | `evolvewp_verifier_` |
| Global classes | `EvolveWP_CJ_` | `EvolveWP_Verifier_` |
| Namespace root | `WPSeed\` | `EvolveWP\Verifier\` |
| Text domain | `wpseed` | `evolvewp-verifier` |
| Option prefix | `evolvewp_cj_` | `evolvewp_verifier_` |
| Hook prefix | `evolvewp_cj_` | `evolvewp_verifier_` |
...
| Type | Convention | Example |
|---|---|---|
| Legacy global class | `EvolveWP_CJ_Category_Name` | `EvolveWP_CJ_Admin_Settings` |
...
```
EVOLVEWP_CJ_VERSION          Plugin version string
EVOLVEWP_CJ_PLUGIN_FILE      Absolute path to main plugin file (__FILE__)
EVOLVEWP_CJ_PLUGIN_DIR_PATH  Absolute path to plugin root directory (trailing slash)
EVOLVEWP_CJ_PLUGIN_URL       Plugin root URL (trailing slash)
EVOLVEWP_CJ_LOG_DIR          Absolute path to log directory
EVOLVEWP_CJ_DEV_MODE         Boolean — true enables developer tooling
```

All constants guarded with `if ( ! defined( 'EVOLVEWP_CJ_CONSTANT_NAME' ) )`.
...
Examples:  evolvewp_cj_plugin_registered
           evolvewp_cj_ecosystem_loaded
           evolvewp_cj_settings_saved

Filter:    {prefix}_{noun}           (returns a value)
Examples:  evolvewp_cj_menu_location
           evolvewp_cj_registered_plugins

AJAX:      {prefix}_{action}
Examples:  evolvewp_cj_save_settings
           evolvewp_cj_get_status
```
...
Examples:  evolvewp_cj_version
           evolvewp_cj_db_version
           evolvewp_cj_ecosystem_mode
           evolvewp_cj_ecosystem_plugins
```
...
Examples:  evolvewp_cj_save_settings
           evolvewp_cj_do_update
           evolvewp_cj_force_update
```
...
Examples:  wp_evolvewp_cj_api_calls
           wp_evolvewp_cj_debug_logs
           wp_evolvewp_cj_notifications
```
...
Examples:  evolvewp-clientjourney-admin
           evolvewp-clientjourney-roadmap
           evolvewp-clientjourney-ecosystem
```
