...
>
> All `wpseed` / `EVOLVEWP_CJ_` / `EvolveWP_CJ_` strings are placeholders. When a new
...
- **7.2** ✅ Replaced all `--tp-` → `--evolvewp-clientjourney-` (46 CSS), `.tp-` → `.evolvewp-clientjourney-` (6 CSS, 1 JS, 7 PHP)
...
- **7.5** ✅ Removed invalid `@extend` from 27 files. Removed duplicate button defs from forms.css. Removed TradePress sections from forms.css. Scoped `reset.css` and `typography.css` to `.evolvewp-clientjourney-admin-wrap`
...
- **Capability Manager** (`Core/Capability_Manager.php`) — Registration with metadata, install/uninstall lifecycle, `evolvewp_cj_user_can()` with filter hook. 7 default caps. Install.php delegates to it. Docs: `docs/CAPABILITIES.md`
...
| `evolvewp_cj_ecosystem()` | Ecosystem Registry singleton | 3.0.0 |
| `evolvewp_cj_log()` | Logger singleton | 3.0.0 |
| `evolvewp_cj_trace()` | Records a trace entry | 3.0.0 |
| `evolvewp_cj_connector()` | API connector instance | 3.1.0 |
| `evolvewp_cj_user_can()` | Capability check (bool) | 3.1.0 |
| `evolvewp_cj_rest_endpoints()` | Registered REST endpoint metadata | 3.1.0 |

---

## Items Deferred to EvolveWP ClientJourney

| Item | Why deferred | Where planned |
|---|---|---|
| Typed Settings Manager | Runtime feature, not boilerplate | EvolveWP ClientJourney Stage B |
| Database Abstraction (Table + Query) | Needed by plugins with custom tables | EvolveWP ClientJourney Stage B |
| Frontend Template System | Needed by plugins with public UI | EvolveWP ClientJourney Stage D |
| Frontend REST API | Public-facing endpoints for portals | EvolveWP ClientJourney Stage D |
| Cloud Services Foundation | Backup, storage, hack detection | EvolveWP ClientJourney Stage D |
| Webhook System | Event notifications to external services | EvolveWP ClientJourney Stage D |
...
| Unmigrated class references | `EvolveWP_CJ_Developer_Mode`, `EvolveWP_CJ_Notifications`, `EvolveWP_CJ_API_Logging` |
