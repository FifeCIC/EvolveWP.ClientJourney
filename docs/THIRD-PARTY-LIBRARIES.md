...
as_schedule_single_action( time() + 3600, 'evolvewp_cj_process_data', array( 'user_id' => 123 ) );

// Schedule a recurring action (every hour)
as_schedule_recurring_action( time(), HOUR_IN_SECONDS, 'evolvewp_cj_hourly_sync' );

// Hook your callback
add_action( 'evolvewp_cj_process_data', 'evolvewp_cj_handle_data_processing' );
function evolvewp_cj_handle_data_processing( $user_id ) {
    // Your processing logic here
    error_log( 'Processing data for user: ' . $user_id );
}

// Check if action is scheduled
if ( as_next_scheduled_action( 'evolvewp_cj_process_data' ) ) {
    // Action is already scheduled
}

// Cancel scheduled action
as_unschedule_action( 'evolvewp_cj_process_data', array( 'user_id' => 123 ) );
```
...
add_action( 'after_setup_theme', 'evolvewp_cj_load_carbon_fields' );
function evolvewp_cj_load_carbon_fields() {
    \Carbon_Fields\Carbon_Fields::boot();
}

// Register fields
add_action( 'carbon_fields_register_fields', 'evolvewp_cj_register_custom_fields' );
function evolvewp_cj_register_custom_fields() {
    
    // Theme Options Page
    Container::make( 'theme_options', __( 'Theme Settings' ) )
        ->add_fields( array(
            Field::make( 'text', 'evolvewp_cj_site_tagline', 'Site Tagline' ),
            Field::make( 'textarea', 'evolvewp_cj_footer_text', 'Footer Text' ),
            Field::make( 'image', 'evolvewp_cj_logo', 'Logo' ),
        ) );
    ...
    Container::make( 'post_meta', __( 'Custom Fields' ) )
        ...
        ->add_fields( array(
            Field::make( 'text', 'evolvewp_cj_subtitle', 'Subtitle' ),
            Field::make( 'checkbox', 'evolvewp_cj_featured', 'Featured Post' ),
            Field::make( 'complex', 'evolvewp_cj_gallery', 'Gallery' )
                ->add_fields( array(
    ...
    Container::make( 'user_meta', __( 'Social Profiles' ) )
        ->add_fields( array(
            Field::make( 'text', 'evolvewp_cj_twitter', 'Twitter URL' ),
            Field::make( 'text', 'evolvewp_cj_linkedin', 'LinkedIn URL' ),
        ) );
}

// Retrieve values
$tagline = carbon_get_theme_option( 'evolvewp_cj_site_tagline' );
$subtitle = carbon_get_post_meta( get_the_ID(), 'evolvewp_cj_subtitle' );
$twitter = carbon_get_user_meta( $user_id, 'evolvewp_cj_twitter' );
```
...
```php
require_once EVOLVEWP_CJ_PLUGIN_DIR_PATH . 'includes/libraries/parsedown/Parsedown.php';
...
class EvolveWP_CJ_Custom_Field extends \Carbon_Fields\Field\Field {
...
### From EvolveWP_CJ_Background_Process to Action Scheduler

**Old Code:**
```php
class My_Background_Process extends EvolveWP_CJ_Background_Process {
...
```php
// Schedule action
as_enqueue_async_action( 'evolvewp_cj_my_process', array( 'id' => 1 ) );

// Hook callback
add_action( 'evolvewp_cj_my_process', 'evolvewp_cj_process_item' );
function evolvewp_cj_process_item( $id ) {
    // Process item
}
```
...
**Old Code:**
```php
add_settings_section( 'evolvewp_cj_section', 'Settings', null, 'evolvewp-clientjourney' );
add_settings_field( 'evolvewp_cj_api_key', 'API Key', 'evolvewp_cj_api_key_callback', 'evolvewp-clientjourney', 'evolvewp_cj_section' );
register_setting( 'evolvewp-clientjourney', 'evolvewp_cj_api_key' );
```
...
Container::make( 'theme_options', 'Settings' )
    ->add_fields( array(
        Field::make( 'text', 'evolvewp_cj_api_key', 'API Key' ),
    ) );
```
