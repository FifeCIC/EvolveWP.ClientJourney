 ...
 *
 * CONSUMED BY:
 *   - functions.php: evolvewp_cj_user_can() global accessor
 *   - EvolveWP ClientJourney\Core\Install: delegates capability installation here
 *   - REST controllers, admin pages, AJAX handlers — anywhere a permission
 *     check is needed
 *   - EvolveWP ClientJourney (elevates this into cross-plugin permission management)
 *
 * DATA FLOW:
 ...
 *     Capability_Manager::register( 'evolvewp_cj_manage_settings', array(
 ...
 *
 *     // Check a capability.
 *     if ( Capability_Manager::user_can( 'evolvewp_cj_manage_settings' ) ) {
 *         // show settings page
 *     }
 *
...
class Capability_Manager {
	 ...
	 *                            plugin prefix (e.g. 'evolvewp_cj_manage_settings').
	 * @param  array  $args {
	...
	/**
	 * Register the default EvolveWP ClientJourney capabilities.
	 *
	 * Called automatically on first use. These are the baseline caps that
	 * every EvolveWP ClientJourney-based plugin starts with. Plugins add their own via
	...
	public static function register_defaults() {
		...
		self::register_many( array(
			'manage_evolvewp-clientjourney' => array(
				'label'       => __( 'Manage EvolveWP ClientJourney', 'evolvewp-clientjourney' ),
				'description' => __( 'Full administrative access to the EvolveWP ClientJourney plugin.', 'evolvewp-clientjourney' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'core',
			),
			'code_evolvewp-clientjourney' => array(
				'label'       => __( 'Code EvolveWP ClientJourney', 'evolvewp-clientjourney' ),
				'description' => __( 'Access developer-level features and code tools.', 'evolvewp-clientjourney' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'development',
			),
			'evolvewp_cj_manage_settings' => array(
				'label'       => __( 'Manage Settings', 'evolvewp-clientjourney' ),
				'description' => __( 'Access and modify the plugin settings page.', 'evolvewp-clientjourney' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'core',
			),
			'evolvewp_cj_view_development' => array(
				'label'       => __( 'View Development Tools', 'evolvewp-clientjourney' ),
				'description' => __( 'Access the Development page with architecture, roadmap, and debug tabs.', 'evolvewp-clientjourney' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'development',
			),
			'evolvewp_cj_manage_connectors' => array(
				'label'       => __( 'Manage API Connectors', 'evolvewp-clientjourney' ),
				'description' => __( 'Configure API credentials and test connector connections.', 'evolvewp-clientjourney' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'api',
			),
			'evolvewp_cj_execute_connectors' => array(
				'label'       => __( 'Execute Connector Actions', 'evolvewp-clientjourney' ),
				'description' => __( 'Run actions on external APIs via the connector system.', 'evolvewp-clientjourney' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'api',
			),
			'evolvewp_cj_view_logs' => array(
				'label'       => __( 'View Logs', 'evolvewp-clientjourney' ),
				'description' => __( 'Access API call logs, debug logs, and trace output.', 'evolvewp-clientjourney' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'development',
			),
		) );
	 ...
	 * allows EvolveWP ClientJourney to intercept and extend permission logic later.
	 *
	 * @since  3.1.0
	 *
	...
	public static function user_can( $capability, $user_id = null ) {
		 ...
		 *
		 * Allows EvolveWP ClientJourney or other plugins to override permission
		 ...
		 */
		$override = apply_filters( 'evolvewp_cj_user_can', null, $capability, $user_id );
	...
	public static function install() {
		 ...
		 */
		do_action( 'evolvewp_cj_capabilities_installed', self::$capabilities );
	}
	...
	public static function uninstall() {
		 ...
		 * @param array<string, array> $capabilities All registered capabilities.
		 */
		do_action( 'evolvewp_cj_capabilities_uninstalled', self::$capabilities );
	}
