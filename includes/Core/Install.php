 ...
 *   - EvolveWP_CJ_Admin_Notices (not yet migrated — referenced via global class name)
 *   - EvolveWP_CJ_Admin_Settings (not yet migrated — referenced via global class name)
 *   - global EvolveWP ClientJourney() function in functions.php
 *
 * CONSUMED BY:
 ...
 *
 * DATA FLOW:
 *   Input  → evolvewp_cj_version option, evolvewp_cj_db_version option
...
namespace EvolveWP\ClientJourney\Core;
...
class Install {
	...
	public static function install() {
		...
		// EvolveWP_CJ_Admin_Notices is not yet namespaced — load via legacy path.
		if ( ! class_exists( 'EvolveWP_CJ_Admin_Notices' ) ) {
			include_once EVOLVEWP_CJ_PLUGIN_DIR_PATH . 'includes/admin/admin-notices.php';
		}
	...
	public static function plugin_row_meta( $links, $file ) {
		if ( $file === EVOLVEWP_CJ_PLUGIN_BASENAME ) {
			$row_meta = array(
				'docs'    => '<a href="' . esc_url( apply_filters( 'evolvewp_cj_docs_url', EVOLVEWP_CJ_DOCS ) ) . '">' . __( 'Docs', 'evolvewp-clientjourney' ) . '</a>',
				'support' => '<a href="' . esc_url( apply_filters( 'evolvewp_cj_support_url', EVOLVEWP_CJ_GITHUB . '/issues' ) ) . '">' . __( 'Support', 'evolvewp-clientjourney' ) . '</a>',
			);
	...
	private static function create_files() {
		...
		$files = array(
			array( 'base' => $upload_dir['basedir'] . '/evolvewp_cj_uploads', 'file' => 'index.html', 'content' => '' ),
			array( 'base' => EVOLVEWP_CJ_LOG_DIR, 'file' => '.htaccess', 'content' => 'deny from all' ),
			array( 'base' => EVOLVEWP_CJ_LOG_DIR, 'file' => 'index.html', 'content' => '' ),
		);

		if ( 'redirect' !== $download_method ) {
			$files[] = array( 'base' => $upload_dir['basedir'] . '/evolvewp_cj_uploads', 'file' => '.htaccess', 'content' => 'deny from all' );
		}
	...
	private static function create_options() {
		if ( ! class_exists( 'EvolveWP_CJ_Admin_Settings' ) ) {
			include_once EVOLVEWP_CJ_PLUGIN_DIR_PATH . 'includes/admin/admin-settings.php';
		}
