...
class EvolveWP_CJ_Listener {
    ...
    private function process_post_requests() {
        if ( ! isset( $_POST['evolvewp_cj_form_action'] ) ) {
            return;
        }
        ...
        $action = sanitize_key( $_POST['evolvewp_cj_form_action'] );
        ...
        do_action( 'evolvewp_cj_process_form_' . $action );
    }
    
    public function display_notices() {
        $notice = get_transient( 'evolvewp_cj_admin_notice' );
        
        if ( $notice ) {
            echo '<div class="notice notice-' . esc_attr( $notice['type'] ) . ' is-dismissible"><p>' . 
                 wp_kses_post( $notice['message'] ) . '</p></div>';
            delete_transient( 'evolvewp_cj_admin_notice' );
        }
    }
}
