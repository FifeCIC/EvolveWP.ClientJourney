<?php
/**
 * EvolveWP ClientJourney Unified Logger
...
class EvolveWP_CJ_Unified_Logger {
...
function evolvewp_cj_js_log_helper() {
    if (!evolvewp_cj_is_developer_mode()) return;
    ?>
    <script>
    window.EvolveWPClientJourneyLogger = {
        ...
        trace: function(type, message, data = {}) {
            ...
            const output = `EvolveWP_CJ_JS: [${type}] ${this.context} (+${timestamp.toFixed(2)}ms): ${message}`;
            ...
            if (Object.keys(data).length > 0) {
                console.log('EvolveWP_CJ_JS_Data:', data);
            }
        },
        ...
        endContext: function(summary = {}) {
            ...
            if (Object.keys(loopSummary).length > 0) {
                console.log('EvolveWP_CJ_JS_Summary: Loops:', loopSummary);
            }
...
// Initialize
add_action('admin_footer', 'evolvewp_cj_js_log_helper');

// Global helper functions
function evolvewp_cj_log() {
    return EvolveWP_CJ_Unified_Logger::instance();
}

function evolvewp_cj_trace($type, $message, $data = array()) {
    EvolveWP_CJ_Unified_Logger::instance()->trace($type, $message, $data);
}