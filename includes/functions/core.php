<?php
/**
 * EvolveWP ClientJourney - Core Functions
 *
...
* function instead.
*/
function evolvewp_cj_is_dashboard() {      
    global $pagenow;
...
*/
function evolvewp_cj_is_background_process() {        
 ...
 */
function evolvewp_cj_print_js() {
    global $evolvewp_cj_queued_js;

    if ( ! empty( $evolvewp_cj_queued_js ) ) {
        // Sanitize.
        $evolvewp_cj_queued_js = wp_check_invalid_utf8( $evolvewp_cj_queued_js );
        $evolvewp_cj_queued_js = preg_replace( '/&#(x)?0*(?(1)27|39);?/i', "'", $evolvewp_cj_queued_js );
        $evolvewp_cj_queued_js = str_replace( "\r", '', $evolvewp_cj_queued_js );

        $js = "<!-- EvolveWP ClientJourney JavaScript -->\n<script type=\"text/javascript\">\njQuery(function($) { $evolvewp_cj_queued_js });\n</script>\n";

        /**
         * evolvewp_cj_queued_js filter.
         *
         * @since 2.6.0
         * @param string $js JavaScript code.
         */
        echo wp_kses( apply_filters( 'evolvewp_cj_queued_js', $js ), array(
            'script' => array( 'type' => array() )
        ) );

        unset( $evolvewp_cj_queued_js );
    }
}
 ...
 * @return string
 */
function evolvewp_cj_help_tip( $tip, $allow_html = false ) {
    if ( $allow_html ) {
        $tip = evolvewp_cj_sanitize_tooltip( $tip );
    } else {
        $tip = esc_attr( $tip );
    }

    return '<span class="evolvewp-clientjourney-help-tip" data-tip="' . $tip . '"></span>';
}
 ...
 * @param string $code
 */
function evolvewp_cj_enqueue_js( $code ) {
    global $evolvewp_cj_queued_js;

    if ( empty( $evolvewp_cj_queued_js ) ) {
        $evolvewp_cj_queued_js = '';
    }
 ...
 * @return bool
 */
function evolvewp_cj_is_developer_mode() {
    return defined('EVOLVEWP_CJ_DEV_MODE') && EVOLVEWP_CJ_DEV_MODE;
}