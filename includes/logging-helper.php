<?php
/**
 * EvolveWP ClientJourney Logging Helper
 * 
...
class EvolveWP_CJ_Logging_Helper {
    ...
    public static function log_api($action, $endpoint, $details = array()) {
        if (get_option('evolvewp_cj_logging_api', 'yes') !== 'yes') {
            return;
        }
    ...
    public static function log_error($error_code, $error_message, $context = array()) {
        if (get_option('evolvewp_cj_logging_errors', 'yes') !== 'yes') {
            return;
        }
    ...
    public static function log_debug($message, $data = null) {
        if (get_option('evolvewp_cj_logging_debug', 'no') !== 'yes') {
            return;
        }
    ...
    private static function write_log($filename, $message) {
        $log_dir = defined('EVOLVEWP_CJ_LOG_DIR') ? EVOLVEWP_CJ_LOG_DIR : WP_CONTENT_DIR . '/evolvewp-clientjourney-logs/';
