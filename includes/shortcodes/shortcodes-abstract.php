...
if ( ! class_exists( 'EvolveWP_CJ_Shortcode' ) ) :
       
abstract class EvolveWP_CJ_Shortcode extends EvolveWP_CJ_Codec_Instance {
    ...
    public function register ($key) {
        ...
        add_action('evolvewp-clientjourney-shortcodes-shortcode_help', array($this, "add_shortcode_help"));
        add_filter('evolvewp-clientjourney-shortcodes-shortcode_help-string', array($this, 'get_shortcode_help'));
    }
    ...
    public function get_shortcode_help ($all='') {             
        ...
        $args_help = '';
        if (!empty($arguments)) {
            $args_help .= '<h4>' . __('Arguments', 'evolvewp-clientjourney') . '</h4><dl>';
        ...
        $help = '' .
                ...
                sprintf(__('Shortcode <code>%s</code>', 'evolvewp-clientjourney'), $this->_key) .
                '</span></h3>' .
                '<div class="inside">' .
                '<h4>' . __('Shortcode', 'evolvewp-clientjourney') . '</h4>' .
                '<pre><code>[' . esc_html($this->_key) . ']</code></pre>' .
                '<h4>' . __('Description', 'evolvewp-clientjourney'). '</h4>' .
                $usage .
                $args_help .
                '</div>' .
                '</div>' .
    ...
    protected function _to_help_string ($arg, $help) {          
        ...
        if (!empty($help['allowed_values'])) {
            $ret[] = __('Allowed values:', 'evolvewp-clientjourney') . ' <code>' . join('</code>, <code>', $help['allowed_values']) . '</code>';
        }
        if (!empty($help['example'])) {
            $ret[] = __('Example:', 'evolvewp-clientjourney') . ' <code>[' . $this->_key . ' ... ' . $arg . '="' . $help['example'] . '"]</code>';
        }
        return join('<br />', $ret);
    }
}
