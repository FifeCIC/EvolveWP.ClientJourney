...
- `evolvewp-clientjourney.pot` - Template file containing all translatable strings
- `evolvewp-clientjourney-{locale}.po` - Translation files for specific languages (e.g., `evolvewp-clientjourney-es_ES.po` for Spanish)
- `evolvewp-clientjourney-{locale}.mo` - Compiled translation files (generated from .po files)
...
```bash
wp i18n make-pot . languages/evolvewp-clientjourney.pot --domain=wpseed
```

Or using the makepot command:

```bash
php wp-content/plugins/evolvewp-clientjourney/bin/makepot.php
```

## Creating a New Translation

1. Copy `evolvewp-clientjourney.pot` to `evolvewp-clientjourney-{locale}.po` (e.g., `evolvewp-clientjourney-fr_FR.po` for French)
...
2. Open `evolvewp-clientjourney.pot`
3. Create new translation from template
4. Save as `evolvewp-clientjourney-{locale}.po`
...
wp i18n make-po languages/evolvewp-clientjourney.pot languages/evolvewp-clientjourney-es_ES.po
