...
/**
 * EvolveWP ClientJourney load order — EvolveWPClientJourney main class.
 *
 * GROUP 1 — Composer PSR-4 autoloader (loaded in evolvewp-clientjourney.php before this file)
 *   vendor/autoload.php — resolves all EvolveWP\ClientJourney\ namespaced classes automatically.
 ...
 *   EvolveWP\ClientJourney\Ecosystem\Registry     — autoloaded via Composer
 ...
 *   EvolveWP\ClientJourney\API\Connector_Interface — autoloaded via Composer
 *   EvolveWP\ClientJourney\API\Base_API             — autoloaded via Composer
 *   EvolveWP\ClientJourney\API\REST_Bridge           — autoloaded via Composer
 *   EvolveWP\ClientJourney\API\REST_Controller      — autoloaded via Composer
...
final class EvolveWPClientJourney {
    
    /**
     * EvolveWP ClientJourney version.
     *
     * @var string
     */
     ...
     * Session instance.
     *
     * @var EvolveWP_CJ_Session
     */
    public $session = null; 
        
    /**
     * Main EvolveWP ClientJourney instance.
     *
     * Ensures only one instance is loaded.
     *
     * @since 1.0.0
     * @static
     * @return EvolveWPClientJourney Main instance.
     */
    ...
    public function __clone() {
        _doing_it_wrong( __FUNCTION__, esc_html__( 'Your not allowed to do that!', 'evolvewp-clientjourney' ), '1.0' );
    }
    ...
    /**
     * EvolveWP ClientJourney constructor.
     *
     * @since 1.0.0
     */
    ...
    private function init_hooks() {
        register_activation_hook( EVOLVEWP_CJ_PLUGIN_FILE, array( '\EvolveWP\ClientJourney\Core\Install', 'install' ) );
        register_deactivation_hook( EVOLVEWP_CJ_PLUGIN_FILE, array( '\EvolveWP\ClientJourney\Core\Install', 'deactivate' ) );
        \EvolveWP\ClientJourney\Core\Install::init();
        \EvolveWP\ClientJourney\Core\AJAX_Handler::init();
        add_action( 'init', array( $this, 'init' ), 0 );
    }

    /**
     * Define EvolveWP ClientJourney constants.
     *
     * @since 1.0.0
     */
    private function define_constants() {

        if ( ! defined( 'EVOLVEWP_CJ_LOG_DIR' ) ) {
            $upload_dir = wp_upload_dir();
            define( 'EVOLVEWP_CJ_LOG_DIR', $upload_dir['basedir'] . '/evolvewp-clientjourney-logs/' );
        }

        if ( ! defined( 'EVOLVEWP_CJ_MIN_WP_VERSION' ) ) { define( 'EVOLVEWP_CJ_MIN_WP_VERSION', $this->min_wp_version ); }
        
        // Main (package) constants.
        if ( ! defined( 'EVOLVEWP_CJ_SESSION_CACHE_GROUP' ) ) { define( 'EVOLVEWP_CJ_SESSION_CACHE_GROUP', 'evolvewp_cj_session_id' ); }
        if ( ! defined( 'EVOLVEWP_CJ_DEV_MODE' ) ) { define( 'EVOLVEWP_CJ_DEV_MODE', false ); }
        if ( ! defined( 'EVOLVEWP_CJ_WORDPRESSORG_SLUG' ) ) { define( 'EVOLVEWP_CJ_WORDPRESSORG_SLUG', false ); }
        if ( ! defined( 'EVOLVEWP_CJ_MARKETPLACE' ) ) { define( 'EVOLVEWP_CJ_MARKETPLACE', false ); }
        if ( ! defined( 'EVOLVEWP_CJ_MARKETPLACE_ID' ) ) { define( 'EVOLVEWP_CJ_MARKETPLACE_ID', false ); }
                                      
        // Project constants.
        if ( ! defined( 'EVOLVEWP_CJ_HOME' ) ) { define( 'EVOLVEWP_CJ_HOME', 'https://evolvewp.dev' ); }
        if ( ! defined( 'EVOLVEWP_CJ_GITHUB' ) ) { define( 'EVOLVEWP_CJ_GITHUB', 'https://github.com/FifeCIC/EvolveWP.ClientJourney' ); }
        if ( ! defined( 'EVOLVEWP_CJ_DOCS' ) ) { define( 'EVOLVEWP_CJ_DOCS', 'https://evolvewp.dev/docs' ); }
    }
    ...
    public function includes() {
        ...
        // Composer PSR-4 autoloader handles all namespaced classes (loaded in evolvewp-clientjourney.php).
        // EvolveWP\ClientJourney\Core\Install is autoloaded via Composer — no include needed.
        // EvolveWP\ClientJourney\Core\AJAX_Handler is autoloaded via Composer — no include needed.
        ...
        // EvolveWP\ClientJourney\Core\Logger is autoloaded via Composer — no include needed.
        // EvolveWP\ClientJourney\Core\Enhanced_Logger is autoloaded via Composer — no include needed.
        ...
        if ( file_exists( plugin_dir_path( __FILE__ ) . 'includes/libraries/action-scheduler/action-scheduler.php' ) ) {
            ...
            // EvolveWP\ClientJourney\Core\Task_Scheduler is autoloaded via Composer.
            \EvolveWP\ClientJourney\Core\Task_Scheduler::instance();
        }
        ...
        // -------------------------------------------------------------------------
        evolvewp_cj_ecosystem(); // boot Registry singleton
        new \EvolveWP\ClientJourney\Ecosystem\Menu_Manager(); // boot menu manager
        new \EvolveWP\ClientJourney\Ecosystem\Installer(); // boot plugin installer
        add_action( 'evolvewp_cj_ecosystem_register', function() {
            evolvewp_cj_ecosystem()->register_plugin( 'evolvewp-clientjourney', array(
                'name'                 => 'EvolveWP ClientJourney',
                'version'              => EVOLVEWP_CJ_VERSION,
                'path'                 => EVOLVEWP_CJ_PLUGIN_DIR_PATH,
                'url'                  => plugins_url( '/', EVOLVEWP_CJ_PLUGIN_FILE ),
                'has_logging'          => true,
                'has_cron'             => true,
        ...
        // EvolveWP\ClientJourney\Admin\Dashboard_Widgets is autoloaded via Composer.
        new \EvolveWP\ClientJourney\Admin\Dashboard_Widgets();
        ...
        // -------------------------------------------------------------------------
        // GROUP 8 — API system (always loaded)
        // EvolveWP\ClientJourney\API\Base_API and EvolveWP\ClientJourney\API\REST_Controller autoloaded via Composer.
        // -------------------------------------------------------------------------
    ...
    public function load_admin_files() {
        ...
        // EvolveWP\ClientJourney\Admin\Notification_Bell autoloaded via Composer.
        \EvolveWP\ClientJourney\Admin\Notification_Bell::init();
        include_once( 'templates/tabs/development/tab-credits.php' );
        include_once( 'toolbars/toolbars.php' );
        // EvolveWP\ClientJourney\Admin\Uninstall_Feedback autoloaded via Composer.
        new \EvolveWP\ClientJourney\Admin\Uninstall_Feedback();
    }

    /**
     * Initialise EvolveWP ClientJourney when WordPress initialises.
     *
     * @since 1.0.0
     */
    public function init() {
        do_action( 'before_evolvewp_cj_init' );
        do_action( 'evolvewp_cj_init' );
    }
