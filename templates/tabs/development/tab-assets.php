<?php
/**
 * EvolveWP ClientJourney Development Assets Tab (Asset Status Management)
 *
...
class EvolveWP_CJ_Admin_Development_Assets {

    public static function output() {
        $asset_manager = self::get_asset_manager();
        if (!$asset_manager) {
            echo '<div class="notice notice-error"><p>' . esc_html__('Asset manager not available.', 'evolvewp-clientjourney') . '</p></div>';
            return;
        }
        ...
        ?>
        <div class="evolvewp-clientjourney-assets-container">
            ...
            <h3><?php esc_html_e('CSS Assets', 'evolvewp-clientjourney'); ?></h3>
            ...
            <h3><?php esc_html_e('JavaScript Assets', 'evolvewp-clientjourney'); ?></h3>
    ...
    private static function get_asset_manager() {
        global $evolvewp_cj_assets;
        return $evolvewp_cj_assets ?? false;
    }

    private static function render_asset_table($assets, $type) {
        if (empty($assets) || !is_array($assets)) {
            echo '<p>' . esc_html__('No assets found.', 'evolvewp-clientjourney') . '</p>';
            return;
        }
        ?>
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    <th><?php esc_html_e('Name', 'evolvewp-clientjourney'); ?></th>
                    <th><?php esc_html_e('Category', 'evolvewp-clientjourney'); ?></th>
                    <th><?php esc_html_e('Purpose', 'evolvewp-clientjourney'); ?></th>
                    <th><?php esc_html_e('Status', 'evolvewp-clientjourney'); ?></th>
                    <th><?php esc_html_e('Path', 'evolvewp-clientjourney'); ?></th>
                    <th><?php esc_html_e('Pages', 'evolvewp-clientjourney'); ?></th>
                    <th><?php esc_html_e('Dependencies', 'evolvewp-clientjourney'); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php
    ...
    private static function check_asset_status($asset, $type) {
        if (!is_array($asset) || empty($asset['path'])) {
            return array('type' => 'error', 'icon' => 'dashicons-no', 'message' => __('Invalid', 'evolvewp-clientjourney'));
        }
        
        if ($asset['path'] === 'external') {
            return array('type' => 'success', 'icon' => 'dashicons-yes', 'message' => __('External', 'evolvewp-clientjourney'));
        }
        
        $asset_path = EVOLVEWP_CJ_PLUGIN_DIR . 'assets/' . ltrim($asset['path'], '/');
        
        if (file_exists($asset_path)) {
            return array('type' => 'success', 'icon' => 'dashicons-yes', 'message' => __('Available', 'evolvewp-clientjourney'));
        } else {
            return array('type' => 'error', 'icon' => 'dashicons-no', 'message' => __('Missing', 'evolvewp-clientjourney'));
        }
    }
    ...
    private static function check_overall_status($css_assets, $js_assets) {
        ...
        if ($total_missing === 0) {
            return array(
                ...
                'icon' => 'dashicons-yes',
                'message' => __('All Assets Available', 'evolvewp-clientjourney'),
                /* translators: %d: Number of assets managed */
                'details' => sprintf(__('%1$d assets managed', 'evolvewp-clientjourney'), $total_assets)
            );
        } elseif ($total_missing <= 3) {
            return array(
                'class' => 'status-warning',
                'icon' => 'dashicons-warning',
                'message' => __('Some Assets Missing', 'evolvewp-clientjourney'),
                /* translators: %1$d: Number of missing assets, %2$d: Total number of assets */
                'details' => sprintf(__('%1$d of %2$d missing', 'evolvewp-clientjourney'), $total_missing, $total_assets)
            );
        } else {
            return array(
                'class' => 'status-error',
                'icon' => 'dashicons-no',
                'message' => __('Many Assets Missing', 'evolvewp-clientjourney'),
                /* translators: %1$d: Number of missing assets, %2$d: Total number of assets */
                'details' => sprintf(__('%1$d of %2$d missing', 'evolvewp-clientjourney'), $total_missing, $total_assets)
            );
        }
...
EvolveWP_CJ_Admin_Development_Assets::output();
