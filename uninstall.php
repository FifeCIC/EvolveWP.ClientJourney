<?php
/**
 * Uninstall EvolveWP ClientJourney.
 *
 ...
 * @author      Ryan Bayne
 * @category    Core
 * @package     EvolveWP ClientJourney/Uninstaller
 * @version     2.0.0
 */
...
$wpdb->query(
    $wpdb->prepare(
        "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s",
        $wpdb->esc_like( '_transient_evolvewp_cj_' ) . '%'
    )
);
$wpdb->query(
    $wpdb->prepare(
        "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s",
        $wpdb->esc_like( '_transient_timeout_evolvewp_cj_' ) . '%'
    )
);
...
wp_clear_scheduled_hook( 'evolvewp_cj_daily_cleanup' );
